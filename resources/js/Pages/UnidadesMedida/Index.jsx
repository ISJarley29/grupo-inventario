...
export default function Index({ auth, unidades }) {
    ...
    return (
        <AuthenticatedLayout user={auth?.user}>
            <Head title="Gestión de Unidades de Medida" />
            ...
            <div className="min-h-screen bg-gray-50 p-8 dark:bg-[#11111d]">
                ...
                <div className="mb-8 flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Gestiona las unidades de medida del sistema.</p>
                    </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-800/50">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Abreviatura</th>
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-[#1a1a27]">
                            {unidades.length === 0 ? (
                                ...
                                unidades.map((uni, index) => (
                                    <tr key={uni.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                        <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                            {index + 1}
                                        </td>
                                        <td className="whitespace-nowrap px-6 py-4">
                                            <div className="text-sm font-medium text-gray-900 dark:text-white">{uni.nombre}</div>
                                        </td>
                                        <td className="whitespace-nowrap px-6 py-4">
                                            <span className="inline-flex rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-medium text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
                                                {uni.abreviatura}
                                            </span>
                                        </td>
            ...
            {/* MODALES EXTERNOS SE MANTIENEN INTACTOS */}
            <ModalNueva
